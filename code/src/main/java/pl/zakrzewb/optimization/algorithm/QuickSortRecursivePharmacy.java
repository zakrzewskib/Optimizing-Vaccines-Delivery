...
public class QuickSortRecursivePharmacy {

    public void sort(ArrayList<Pharmacy> unsortedVector) {
    ...
    private void quicksort(ArrayList<Pharmacy> data) {
        int left = 0;
        int right = data.size() - 1;
        quicksort(data, left, right);
    }

    private void quicksort(ArrayList<Pharmacy> data, int left, int right) {
        if (left < right) {
    ...
    private int splitData(ArrayList<Pharmacy> data, int start, int end) {
        int left = start + 1;
        int right = end;
    ...
    private void swap(ArrayList<Pharmacy> data, int firstId, int secondId) {
        if (firstId != secondId) {
            Pharmacy firstValue = data.get(firstId);
